...
def loadERIs_fock(fcidump,ncore,nact):
   ...
   with open(fcidump,'r') as f:
     ...
     for line in f.readlines():
       ...
       if ind[0] > n-1 or ind[1] > n-1 or ind[2] > n-1 or ind[3] > n-1:
         continue 
...
def loadERIs2(fcidump):
   #print ('\n[tools_io.loadERIs] from FCIDUMP')
   with open(fcidump,'r') as f:
     line = f.readline().split(',')
     norb = int(line[0].split(' ')[-1])
     #print  ('Num of orb: ', norb)
     nelec= int(line[1].split('=')[-1])
     #print  ('Num of elec: ', nelec)
